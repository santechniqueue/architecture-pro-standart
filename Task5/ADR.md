...
1. Встроить кредитный процесс в монолит интернет-банка:<br>
Плюсы: быстрее старт, меньше DevOps.<br>
Минусы: низкая масштабируемость, высокий техдолг, сложные релизы.<br>
Когда выбирать: дедлайн критичен и нагрузка низкая.
2. Единая технологическая платформа .NET - скоринг переносится в .NET:<br>
Плюсы: единый рантайм; меньше межязыковых интеграций; проще сопровождение.<br>
Минусы: потеря гибкости Python-стека; миграционные затраты.<br>
Когда: приоритет TCO/унификация, скоринг относительно прост.<br>
3. Единый Credit Service без Camunda:<br>
Плюсы: меньше технологий и команд; проще DevOps; один источник бизнес-правил.
Минусы: долгая разработка.
Когда: требование минимизировать стек и высокое время вывода.

**"Недостатки, ограничения, риски"**

1. Интеграция с БКИ - внешний SLA/лимиты:<br>
Риск: таймауты, блокировки, деградация UX и конвейера.
Исправление: retry с backoff+jitter, rate-limit, кэш последнего валидного ответа, очереди отложенной обработки.
2. СМС через один провайдер. <br>
Риск: недоставка/аварии провайдера > блокирующий стоп-поток.
Исправление: 2 провайдера, очередь повторов.
